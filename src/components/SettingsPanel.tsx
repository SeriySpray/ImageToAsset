...
import { HalftoneSettings, TornEdgeSettings, HalftonePattern } from '../types';
...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({
  ...
  const patterns: { id: HalftonePattern; name: string; icon: React.ReactNode }[] = [
      ...
      <div className="p-4 border-b border-slate-800/80">
        ...
        <div className="mb-4">
          <label className="text-[11px] font-medium text-slate-400 mb-1.5 block">
            Тип графічного растру
          </label>
          <div className="grid grid-cols-3 gap-1 bg-slate-900/90 p-1 rounded-xl border border-slate-800">
            {patterns.map((p) => {
              const active = halftone.pattern === p.id;
              return (
                <button
                  key={p.id}
                  onClick={() => onChangeHalftone({ pattern: p.id })}
